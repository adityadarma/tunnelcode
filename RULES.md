...
- After finishing a task, new feature or fix bug, always run `pnpm test && pnpm typecheck && pnpm lint && pnpm format:check && pnpm build` from the workspace root to verify the whole project compiles cleanly before presenting the result.
