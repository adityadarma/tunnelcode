...
- After finishing a task, new feature or fix bug, always run `pnpm typecheck && pnpm format && pnpm lint && pnpm test && pnpm build` from the workspace root to verify the whole project compiles cleanly before presenting the result.
