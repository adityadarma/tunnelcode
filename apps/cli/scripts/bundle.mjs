 ...
 */
const DEFAULT_SERVER_URL = process.env['TUNNELCODE_DEFAULT_SERVER_URL'] ?? 'http://localhost:3000';
...
async function writeManifest(manifest) {
  ...
  const published = {
    ...
    repository: {
      type: 'git',
      url: 'git+https://github.com/adityadarma/tunnelcode.git',
    },
    homepage: 'https://github.com/adityadarma/tunnelcode#readme',
    bugs: { url: 'https://github.com/adityadarma/tunnelcode/issues' },
    // No "./" prefix: npm 11 treats it as an invalid bin script name and strips
    // the entry from the manifest it publishes, warning on every publish.
    bin: { tunnelcode: 'index.js' },
    files: ['index.js'],
    engines: manifest.engines ?? { node: '>=22.18.0' },
...
await build({
  ...
  define: {
    ...
    TUNNELCODE_BUNDLED_SERVER_URL: JSON.stringify(DEFAULT_SERVER_URL),
    TUNNELCODE_BUNDLED_VERSION: JSON.stringify(manifest.version),
  },
  logLevel: 'info',
});
