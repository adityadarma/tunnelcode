...
import { parseCliMessage } from '@remotecode/protocol';
import type { CliMessage, ServerToCliMessage } from '@remotecode/protocol';
  ...
  app.get('/ws/cli', { websocket: true }, (socket: WebSocket) => {
    ...
    const handle = (message: CliMessage): void => {
      switch (message.type) {
            ...
            reply({
              ...
              message:
                result.reason === 'workspace_busy'
                  ? 'This workspace is already running an agent. Stop the other remotecode first.'
        ...
        case 'turn_activity':
          if (deviceId !== undefined) {
            relay.activity(deviceId, message.turnId, message.tool, message.target);
          }
          return;
