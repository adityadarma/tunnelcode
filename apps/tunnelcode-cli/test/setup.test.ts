...
import { globalConfigPath, loadGlobalConfig } from '@tunnelcode/config';
...
test('the default server url uses localhost and port 3000', async () => {
  await withTempHome(async () => {
    await withEnv(
      { HOST: undefined, PORT: undefined, TUNNELCODE_SERVER_URL: undefined },
      async () => {
...
test('PORT changes the default server url', async () => {
  await withTempHome(async () => {
    await withEnv({ HOST: undefined, PORT: '8080', TUNNELCODE_SERVER_URL: undefined }, async () => {
      await runSetup({ force: true });
...
test('HOST changes the default server url', async () => {
  await withTempHome(async () => {
    await withEnv(
      { HOST: 'rc.internal', PORT: '9000', TUNNELCODE_SERVER_URL: undefined },
      async () => {
        await runSetup({ force: true });
...
test('a bind address of 0.0.0.0 falls back to localhost', async () => {
  await withTempHome(async () => {
    await withEnv({ HOST: '0.0.0.0', PORT: '3000', TUNNELCODE_SERVER_URL: undefined }, async () => {
      await runSetup({ force: true });
...
test('TUNNELCODE_SERVER_URL wins over host and port', async () => {
  await withTempHome(async () => {
    await withEnv(
      { HOST: 'ignored', PORT: '1234', TUNNELCODE_SERVER_URL: 'https://rc.example.com' },
      async () => {
        await runSetup({ force: true });
...
test('an explicit --server flag wins over the environment', async () => {
  await withTempHome(async () => {
    await withEnv(
      { HOST: 'env.host', PORT: '1234', TUNNELCODE_SERVER_URL: 'https://env.example.com' },
      async () => {
