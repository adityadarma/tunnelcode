...
export async function run(argv: readonly string[]): Promise<number> {
  ...
  try {
    ...
    writeErr('Run remotecode --help to see available options.');
    return 1;
  }
  ...
  // No command means start, so `remotecode` alone starts the agent.
  const name = positionals[0] ?? 'start';

  if (!isCommand(name)) {
    writeErr(`Unknown command: ${name}`);
    writeErr('Run remotecode --help to see available commands.');
    return 1;
  }
