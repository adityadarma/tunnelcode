 ...
 */
declare const REMOTECODE_BUNDLED_SERVER_URL: string | undefined;

function bundledServerUrl(): string | undefined {
  // Guarded because the identifier only exists in a bundled build.
  const value =
    typeof REMOTECODE_BUNDLED_SERVER_URL === 'string' ? REMOTECODE_BUNDLED_SERVER_URL : undefined;
 ...
 * 2. REMOTECODE_SERVER_URL, an explicit choice for this environment
...
export function serverUrlFromEnvironment(): string | undefined {
  const explicit = process.env['REMOTECODE_SERVER_URL'];
