...
async function withFixture<T>(run: (fixture: Fixture) => Promise<T>): Promise<T> {
  const home = await mkdtemp(join(tmpdir(), 'remotecode-stop-home-'));
  const binDir = await mkdtemp(join(tmpdir(), 'remotecode-stop-bin-'));

  const enginePath = join(binDir, 'opencode');
  await writeFile(enginePath, FAKE_ENGINE, 'utf8');
  await chmod(enginePath, 0o755);

  const configDir = join(home, '.config', 'remotecode');
  await mkdir(configDir, { recursive: true });
  await writeFile(
    join(configDir, 'remotecode.json'),
    JSON.stringify({
...
function startCli(fixture: Fixture, serverUrl: string): ChildProcess {
  return spawn(process.execPath, ['./dist/index.js'], {
    ...
    env: {
      ...
      REMOTECODE_SERVER_URL: serverUrl,
