...
const clean = {
  REMOTECODE_SERVER_URL: undefined,
  HOST: undefined,
  PORT: undefined,
};
...
test('an empty variable does not override a stored config', () => {
  withEnv({ REMOTECODE_SERVER_URL: '', HOST: '', PORT: '' }, () => {
    assert.equal(serverUrlFromEnvironment(), undefined);
  });
});
...
test('REMOTECODE_SERVER_URL wins over host and port', () => {
  withEnv(
    { REMOTECODE_SERVER_URL: 'https://rc.example.com', HOST: 'ignored', PORT: '1234' },
    () => {
